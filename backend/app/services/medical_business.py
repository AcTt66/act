...
def build_natural_system_prompt(scene: str) -> str:
    if scene == "triage":
        ...
        content_order = """回答的内容顺序：
1. 先理解并总结用户的主要症状和基本情况。
2. 给出 2-3 种可能的情况分析，用"考虑"、"可能"、"不排除"这样的措辞。
3. 明确给出**风险等级**（低风险 / 中风险 / 高风险），并说明理由。
4. 明确推荐下一步去哪个科室就诊（用"建议挂号 XX 科"这样的口吻）。
5. 提供就医边界建议。"""
...
3. 提供生活护理和就医建议。"""
        ...
        focus = "重点是健康科普问诊：做症状梳理、报告解读、健康建议。**不必须推荐科室，重点在于提供专业的健康科普和报告解读**。"
        content_order = """回答的内容顺序：
1. 如果用户上传了报告/影像，先解读本轮报告的关键信息，明确异常项、正常项和临床意义。
2. 如果系统提供了相关历史报告，再说明它们与本轮资料的关系：新增、消失、减轻、加重、稳定或资料不足；不要混淆当前与既往。
3. 结合患者基础情况和当前描述分析症状。
4. 给出 2-4 种可能的原因或情况分析，用"考虑"、"可能"、"不排除"这样的措辞。
5. 提供生活护理 / 自我观察 / 是否需要就医的建议，**不必须推荐科室，但如果有明显的科室指向，可以用"可考虑咨询 XX 科进一步就诊"这样的建议，不强制。
6. 如果存在红旗症状（如剧烈胸痛、呼吸困难、大量出血、意识改变、持续高热不退等），明确警示需要及时线下就医或急诊。
7. 如果还有信息不清楚的地方，最后追问 0-4 个问题；如果信息已经够充分就不问。"""

    base_prompt = f"""你是互联网医院的医疗助手 AI。{focus}请用自然、温和、清晰的中文回答，像医生面诊那样有温度，禁止确诊、禁止开处方。
...
风格要求：
- 自然有温度的医生口吻，不要用"结论："、"依据与原因："这类刻板的固定标题分段。
...
<meta>{{"risk_level":"低风险或中风险或高风险","recommended_department":"{"必须从下面列表里精确选择一个" if scene == "triage" else "可选，如果没有明确的科室推荐可以填普通内科"}"}}</meta>
...
def build_local_fallback_answer(message: str, risk_hint: Dict[str, Any], department: str, scene: str = "triage") -> str:
    """远程模型完全不可用时的最小安全回答，保证用户仍拿到基本就医边界，根据场景差异化处理。"""
    ...
    if scene == "triage":
        # 智能分诊场景
        lines = [
            "远程模型暂时连接不稳定，我先给你一个基础的分诊建议。",
            f"你描述的是：{message[:120]}",
            f"从本地规则看，当前先按 **{risk}** 处理；建议优先咨询或挂号 **{department}**。",
        ]
        # 添加meta标签 - 分诊场景需要明确推荐科室
        meta_tag = f'<meta>{{"risk_level":"{risk}","recommended_department":"{department}"}}</meta>'
    elif scene == "medication":
        # 用药咨询场景
        lines = [
            "远程模型暂时连接不稳定，我先给你一个基础的用药安全建议。",
            f"你咨询的是：{message[:120]}",
            f"对于用药相关问题，我建议你先咨询 **{department}** 或药师，以获得专业的指导。",
        ]
        # 用药咨询场景默认推荐药学门诊
        meta_tag = f'<meta>{{"risk_level":"{risk}","recommended_department":"{department}"}}</meta>'
    else:
        ...
        lines = []
        # 检测是否是报告分析场景
        is_report_analysis = any(keyword in message for keyword in ["报告", "帮我", "分析", "解读", "看看"])
        if is_report_analysis:
            lines.append("你的报告已成功上传。")
            lines.append("虽然远程模型暂时连接不稳定，我无法为你进行AI自动分析，但：")
            lines.append("1. 你的报告已安全保存在系统中，可以在「报告查询」中查看")
            lines.append("2. 建议你稍后再试，或者拿着报告咨询专业医生")
        else:
            lines.append("远程模型暂时连接不稳定，我先给你一个基础的健康建议。")
            lines.append(f"你描述的是：{message[:120]}")
            lines.append("建议你可以先观察，有需要时咨询相关科室。")
        # 健康咨询场景 - 不推荐科室
        meta_tag = f'<meta>{{"risk_level":"{risk}"}}</meta>'
...
"""
    ...
    async def run_stream(
        ...
        try:
            ...
            err_text = build_local_fallback_answer(message, risk_hint, department, scene)
