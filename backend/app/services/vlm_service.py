...
class VLMService:
    ...
    async def _chat_completions_analyze(self, images: List[ImagePayload], prompt: str, timeout: float) -> str:
        # OpenAI 标准格式：使用 text 和 image_url 类型
        image_contents = [
            {"type": "image_url", "image_url": {"url": img.data_url}}
            for img in images
        ]

        async def _send_request(content_list: List[Dict[str, Any]]) -> str:
            payload = {
                "model": self.model_name,
                "messages": [
                    {"role": "system", "content": SYSTEM_PROMPT},
                    {"role": "user", "content": [{"type": "text", "text": prompt}, *content_list]},
                ],
                "temperature": self.temperature,
                "max_tokens": self.max_tokens,
            }
            ...
            async with httpx.AsyncClient(
                ...
                if resp.status_code != 200:
                    logger.error("VLM API error: status=%s body=%s", resp.status_code, resp.text[:500])
                resp.raise_for_status()
                data = resp.json()
        ...
        for attempt in range(3):
            try:
                return await _send_request(image_contents)
            except Exception as exc:
                last_error = exc
                logger.warning("VLM image parse attempt %s failed: %s", attempt + 1, exc)
            await asyncio.sleep(0.8 + attempt * 1.2)
        raise RuntimeError(f"VLM analyze failed: {last_error}")
...
def build_failed_report(error: Exception, doc_type_hint: Optional[str] = None) -> Dict[str, Any]:
    ...
    message = f"原始文件已保存，等待AI解析。错误类型：{type(error).__name__}"
    return {
        "doc_type": doc_type,
        "title": "医疗报告",
        "report_date": None,
