...
async def quick_test():
    ...
    if not vlm.enabled:
        print("[FAIL] VLM not enabled!")
        return False
    
    # 2. 创建测试文件
    from pathlib import Path
    test_file = Path("./test_small.jpg")
    ...
    test_file.write_bytes(minimal_jpg)
    print(f"Test file created: {test_file} ({len(minimal_jpg)} bytes)")
    ...
    try:
        ...
        print(f"[FAIL] Document processor error: {e}")
        import traceback
        traceback.print_exc()
        return False
