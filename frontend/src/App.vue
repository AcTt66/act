<template>
  ...
  <div v-else class="layout">
    ...
    <main class="main">
      ...
      <!-- 健康数据记录 -->
      <section v-if="activePage === 'health'" class="panel">
        <div class="panel-title">
          <h2>健康数据记录</h2>
          <el-button type="primary" @click="openAddHealthRecord">+ 添加记录</el-button>
        </div>
        <div class="filter-bar">
          <el-radio-group v-model="healthRecordType" @change="loadHealthRecords">
            <el-radio-button label="">全部</el-radio-button>
            <el-radio-button label="血压">血压</el-radio-button>
            <el-radio-button label="血糖">血糖</el-radio-button>
            <el-radio-button label="体重">体重</el-radio-button>
          </el-radio-group>
        </div>
        <div class="record-cards">
          <article v-for="record in healthRecords" :key="record.id" class="record-card">
            <div class="record-top">
              <el-tag :type="record.record_type === '血压' ? 'danger' : record.record_type === '血糖' ? 'warning' : 'success'">{{ record.record_type }}</el-tag>
              <span>{{ formatTime(record.recorded_at || record.created_at) }}</span>
            </div>
            <div class="record-chief">{{ record.value }} {{ record.unit }}</div>
            <div v-if="record.assessment_result" class="assessment-badge">
              <el-tag :type="record.assessment_result === '正常' ? 'success' : record.assessment_result === '偏高' ? 'warning' : 'danger'">
                {{ record.assessment_result }}
              </el-tag>
            </div>
            <p v-if="record.advice" class="advice-text">{{ record.advice }}</p>
            <p v-if="record.note">{{ record.note }}</p>
            <el-button size="small" type="danger" plain @click="removeHealthRecord(record.id)">删除</el-button>
          </article>
        </div>
        <div v-if="!healthRecords.length" class="empty-state">暂无健康记录，点击上方按钮添加。</div>

        <el-dialog v-model="healthDialogVisible" title="添加健康记录" width="500px">
          <el-form label-position="top">
            <el-form-item label="记录类型">
              <el-select v-model="healthForm.record_type" placeholder="选择类型">
                <el-option label="血压 (mmHg)" value="血压" />
                <el-option label="血糖 (mmol/L)" value="血糖" />
                <el-option label="体重 (kg)" value="体重" />
              </el-select>
            </el-form-item>
            <el-form-item label="数值">
              <el-input-number v-model="healthForm.value" :min="0" :precision="1" />
            </el-form-item>
            <el-form-item label="关联成员">
              <el-select v-model="healthForm.family_member_id" placeholder="选择家庭成员（可选）" clearable>
                <el-option v-for="member in familyMembers" :key="member.id" :label="member.name" :value="member.id" />
              </el-select>
            </el-form-item>
            <el-form-item label="备注">
              <el-input v-model="healthForm.note" placeholder="可选备注" />
            </el-form-item>
          </el-form>
          <template #footer>
            <el-button @click="healthDialogVisible = false">取消</el-button>
            <el-button type="primary" :loading="healthLoading" @click="submitHealthRecord">保存</el-button>
          </template>
        </el-dialog>
      </section>

      <!-- 家庭成员管理 -->
      <section v-if="activePage === 'family'" class="panel">
        <div class="panel-title">
          <h2>家庭成员</h2>
          <el-button type="primary" @click="openAddFamilyMember">+ 添加成员</el-button>
        </div>
        <div class="family-grid">
          <article v-for="member in familyMembers" :key="member.id" class="family-card">
            <div class="family-avatar" :style="{ backgroundColor: member.avatar_color || getAvatarColor(member.id) }">
              {{ member.name?.slice(0, 1) || '?' }}
            </div>
            <div class="family-info">
              <strong>{{ member.name }}</strong>
              <span>{{ member.relation || '家庭成员' }} · {{ member.gender || '' }} {{ member.age ? member.age + '岁' : '' }}</span>
              <p v-if="member.chronic_diseases">慢病：{{ member.chronic_diseases }}</p>
              <p v-if="member.allergy_history">过敏：{{ member.allergy_history }}</p>
              <div v-if="member.health_summary" class="health-summary">
                <el-tag size="small" type="info">📊 {{ member.health_summary }}</el-tag>
              </div>
              <div v-if="member.checkin_days" class="checkin-stats-badge">
                <span>7天打卡: {{ member.checkin_days }}次</span>
              </div>
            </div>
            <div class="family-actions">
              <el-button size="small" @click="editFamilyMember(member)">编辑</el-button>
              <el-button size="small" type="danger" plain @click="removeFamilyMember(member.id)">删除</el-button>
            </div>
          </article>
        </div>
        <div v-if="!familyMembers.length" class="empty-state">暂无家庭成员，点击上方按钮添加。</div>

        <el-dialog v-model="familyDialogVisible" :title="editingFamilyMember ? '编辑成员' : '添加成员'" width="550px">
          <el-form label-position="top">
            <el-form-item label="姓名"><el-input v-model="familyForm.name" /></el-form-item>
            <el-form-item label="性别">
              <el-select v-model="familyForm.gender" clearable><el-option label="男" value="男" /><el-option label="女" value="女" /></el-select>
            </el-form-item>
            <el-form-item label="年龄"><el-input-number v-model="familyForm.age" :min="0" :max="130" /></el-form-item>
            <el-form-item label="关系"><el-input v-model="familyForm.relation" placeholder="如：父亲、母亲、子女" /></el-form-item>
            <el-form-item label="电话"><el-input v-model="familyForm.phone" /></el-form-item>
            <el-form-item label="慢病史"><el-input v-model="familyForm.chronic_diseases" type="textarea" /></el-form-item>
            <el-form-item label="过敏史"><el-input v-model="familyForm.allergy_history" type="textarea" /></el-form-item>
          </el-form>
          <template #footer>
            <el-button @click="familyDialogVisible = false">取消</el-button>
            <el-button type="primary" @click="submitFamilyMember">{{ editingFamilyMember ? '保存' : '添加' }}</el-button>
          </template>
        </el-dialog>
      </section>

      <!-- 健康打卡 -->
      <section v-if="activePage === 'checkin'" class="panel">
        <div class="panel-title">
          <h2>健康打卡</h2>
          <el-button type="primary" @click="openCheckinDialog">+ 今日打卡</el-button>
        </div>
        <div class="checkin-stats">
          <div class="stat-item"><strong>{{ checkins.length }}</strong><span>本月打卡次数</span></div>
          <div class="stat-item"><strong>{{ checkinStats }}</strong><span>连续打卡天数</span></div>
        </div>
        <div class="checkin-grid">
          <button v-for="type in checkinTypes" :key="type" class="checkin-quick-btn" @click="quickCheckin(type)">
            <span>{{ type }}</span>
          </button>
        </div>
        <div class="checkin-list">
          <article v-for="item in checkins" :key="item.id" class="checkin-item">
            <el-tag :type="getCheckinTagType(item.checkin_type)">{{ item.checkin_type }}</el-tag>
            <span>{{ formatTime(item.checked_at || item.created_at) }}</span>
            <span v-if="item.streak_days && item.streak_days > 1" class="streak-badge">🔥 {{ item.streak_days }}天</span>
            <span v-if="item.mood" class="mood-badge">心情: {{ item.mood }}</span>
            <span v-if="item.note">{{ item.note }}</span>
            <el-button size="small" type="danger" plain @click="removeCheckin(item.id)">删除</el-button>
          </article>
        </div>
        <div v-if="!checkins.length" class="empty-state">暂无打卡记录，开始今日打卡吧！</div>

        <el-dialog v-model="checkinDialogVisible" title="健康打卡" width="450px">
          <el-form label-position="top">
            <el-form-item label="打卡类型">
              <el-select v-model="checkinForm.checkin_type" placeholder="选择类型">
                <el-option v-for="t in checkinTypes" :key="t" :label="t" :value="t" />
              </el-select>
            </el-form-item>
            <el-form-item label="心情">
              <el-select v-model="checkinForm.mood" placeholder="选择心情（可选）" clearable>
            ...
            <el-form-item label="关联成员">
              <el-select v-model="checkinForm.family_member_id" placeholder="选择家庭成员（可选）" clearable>
                <el-option v-for="member in familyMembers" :key="member.id" :label="member.name" :value="member.id" />
              </el-select>
            </el-form-item>
            <el-form-item label="备注"><el-input v-model="checkinForm.note" placeholder="可选备注" /></el-form-item>
          </el-form>
          <template #footer>
            <el-button @click="checkinDialogVisible = false">取消</el-button>
            <el-button type="primary" :loading="checkinLoading" @click="submitCheckin">打卡</el-button>
          </template>
        </el-dialog>
      </section>

      <!-- 我的收藏 -->
      <section v-if="activePage === 'favorites'" class="panel">
        <div class="panel-title">
          <h2>我的收藏</h2>
          <el-button type="primary" @click="favoriteDialogVisible = true">+ 添加收藏</el-button>
        </div>
        <div class="filter-bar">
          <el-radio-group v-model="favoriteType" @change="loadFavorites">
            <el-radio-button label="">全部</el-radio-button>
            <el-radio-button label="知识">知识</el-radio-button>
            <el-radio-button label="科室">科室</el-radio-button>
            <el-radio-button label="报告">报告</el-radio-button>
          </el-radio-group>
        </div>
        <div class="record-cards">
          <article v-for="fav in favorites" :key="fav.id" class="record-card">
            <div class="record-top">
              <el-tag>{{ fav.fav_type }}</el-tag>
              <span>{{ formatTime(fav.created_at) }}</span>
            </div>
            <div class="record-chief">{{ fav.title || '未命名收藏' }}</div>
            <p v-if="fav.content">{{ fav.content }}</p>
            <el-button size="small" type="danger" plain @click="removeFavorite(fav.id)">删除</el-button>
          </article>
        </div>
        <div v-if="!favorites.length" class="empty-state">暂无收藏内容。</div>

        <el-dialog v-model="favoriteDialogVisible" title="添加收藏" width="400px">
          <el-form label-position="top">
            <el-form-item label="收藏类型">
              <el-select v-model="favoriteForm.fav_type" placeholder="选择类型">
            ...
            <el-form-item label="内容"><el-input v-model="favoriteForm.content" type="textarea" :rows="3" /></el-form-item>
          </el-form>
          <template #footer>
            <el-button @click="favoriteDialogVisible = false">取消</el-button>
            <el-button type="primary" @click="submitFavorite">收藏</el-button>
          </template>
        </el-dialog>
      </section>

      <!-- 消息通知 -->
      <section v-if="activePage === 'notifications'" class="panel">
        <div class="panel-title">
          <h2>消息通知</h2>
          <div class="panel-actions">
            <el-radio-group v-model="notificationType" @change="loadNotifications" size="small">
              <el-radio-button label="">全部</el-radio-button>
              <el-radio-button label="health">健康警告</el-radio-button>
              <el-radio-button label="appointment">预约提醒</el-radio-button>
              <el-radio-button label="system">系统消息</el-radio-button>
            </el-radio-group>
            <el-button v-if="unreadCount > 0" type="primary" plain @click="markAllRead">全部标为已读</el-button>
          </div>
        </div>
        <div class="notification-badge" v-if="unreadCount > 0">
          <el-badge :value="unreadCount" type="danger">未读消息</el-badge>
        </div>
        <div class="notification-list">
          <article v-for="notif in notifications" :key="notif.id" class="notification-item" :class="{ unread: !notif.is_read, 'high-priority': notif.priority === 'high' }">
            <div class="notif-icon" :class="{ 'warning-icon': notif.priority === 'high' }">{{ getNotifIcon(notif.notif_type || notif.type) }}</div>
            <div class="notif-content">
              <div class="notif-header">
                <strong>{{ notif.title }}</strong>
                <el-tag v-if="notif.priority === 'high'" type="danger" size="small">紧急</el-tag>
                <el-tag v-if="notif.notif_type || notif.type" size="small" type="info">{{ getNotifTypeName(notif.notif_type || notif.type) }}</el-tag>
              </div>
              <p v-if="notif.content">{{ notif.content }}</p>
              <div v-if="notif.family_member_name" class="notif-member">
                <span>👤 {{ notif.family_member_name }}</span>
              </div>
              <small>{{ formatTime(notif.created_at) }}</small>
            </div>
            <div class="notif-actions">
              <el-button v-if="!notif.is_read" size="small" plain @click="markRead(notif.id)">已读</el-button>
              <el-button size="small" type="danger" plain @click="removeNotification(notif.id)">删除</el-button>
            </div>
          </article>
        </div>
        <div v-if="!notifications.length" class="empty-state">暂无通知消息。</div>
      </section>

      <!-- 健康工具 -->
      <section v-if="activePage === 'tools'" class="grid">
        <div class="panel">
          <h2>BMI 计算器</h2>
          <p class="muted">计算身体质量指数，评估体重是否健康。</p>
          <el-form label-position="top">
            <el-form-item label="身高 (cm)"><el-input-number v-model="toolForm.height" :min="50" :max="250" /></el-form-item>
            <el-form-item label="体重 (kg)"><el-input-number v-model="toolForm.weight" :min="20" :max="300" :precision="1" /></el-form-item>
          </el-form>
          <div class="tool-actions">
            <el-button type="primary" @click="calcBMI">计算 BMI</el-button>
            <el-button v-if="bmiResult" type="success" plain @click="saveBMIResult">保存到健康记录</el-button>
          </div>
          <div v-if="bmiResult" class="tool-result">
            <div class="bmi-value">BMI: {{ bmiResult.bmi }}</div>
            <el-tag :type="bmiResult.category === '正常' ? 'success' : bmiResult.category === '偏瘦' ? 'info' : 'warning'">{{ bmiResult.category }}</el-tag>
            <p>{{ bmiResult.advice }}</p>
          </div>
        </div>
        <div class="panel">
          <h2>血压评估</h2>
          <p class="muted">评估血压水平，了解健康状态。</p>
          <el-form label-position="top">
            <el-form-item label="收缩压 (mmHg)"><el-input-number v-model="toolForm.systolic" :min="60" :max="250" /></el-form-item>
            <el-form-item label="舒张压 (mmHg)"><el-input-number v-model="toolForm.diastolic" :min="40" :max="150" /></el-form-item>
          </el-form>
          <div class="tool-actions">
            <el-button type="primary" @click="calcBP">评估血压</el-button>
            <el-button v-if="bpResult" type="success" plain @click="saveBPResult">保存到健康记录</el-button>
          </div>
          <div v-if="bpResult" class="tool-result">
            <el-tag :type="bpResult.category === '正常' ? 'success' : 'warning'">{{ bpResult.category }}</el-tag>
            <p>{{ bpResult.advice }}</p>
          </div>
        </div>
        <div class="panel">
          <h2>血糖评估</h2>
          <p class="muted">评估血糖水平，了解健康状态。</p>
          <el-form label-position="top">
            <el-form-item label="血糖值 (mmol/L)"><el-input-number v-model="toolForm.bloodGlucose" :min="1" :max="30" :precision="1" /></el-form-item>
            <el-form-item label="测量时间">
              <el-select v-model="toolForm.glucoseTime" placeholder="选择时间">
                <el-option label="空腹" value="空腹" />
                <el-option label="餐后2小时" value="餐后2小时" />
                <el-option label="随机" value="随机" />
              </el-select>
            </el-form-item>
          </el-form>
          <div class="tool-actions">
            <el-button type="primary" @click="calcBloodGlucose">评估血糖</el-button>
            <el-button v-if="glucoseResult" type="success" plain @click="saveGlucoseResult">保存到健康记录</el-button>
          </div>
          <div v-if="glucoseResult" class="tool-result">
            <el-tag :type="glucoseResult.category === '正常' ? 'success' : 'warning'">{{ glucoseResult.category }}</el-tag>
            <p>{{ glucoseResult.advice }}</p>
          </div>
        </div>
        <div class="panel">
          <h2>健康目标</h2>
          <p class="muted">设置健康目标，追踪完成进度。</p>
          <div class="goal-list">
            <div v-for="goal in healthGoals" :key="goal.id" class="goal-item">
              ...
              <p class="goal-target">目标: {{ goal.target }}{{ goal.unit }}</p>
              <p class="goal-current">当前: {{ goal.current }}{{ goal.unit }}</p>
            </div>
          </div>
          <el-button type="primary" plain @click="openGoalDialog">+ 添加目标</el-button>
        </div>
      </section>

      <el-dialog v-model="goalDialogVisible" title="添加健康目标" width="450px">
        <el-form label-position="top">
          <el-form-item label="目标名称">
            <el-select v-model="goalForm.name" placeholder="选择目标类型">
              <el-option label="每日步数" value="每日步数" />
              <el-option label="每日饮水量" value="每日饮水量" />
              <el-option label="每周运动次数" value="每周运动次数" />
              <el-option label="每日睡眠小时" value="每日睡眠小时" />
            </el-select>
          </el-form-item>
          <el-form-item label="目标值"><el-input-number v-model="goalForm.target" :min="1" /></el-form-item>
          <el-form-item label="单位"><el-input v-model="goalForm.unit" placeholder="如：步、ml、次" /></el-form-item>
        </el-form>
        <template #footer>
          <el-button @click="goalDialogVisible = false">取消</el-button>
          <el-button type="primary" @click="submitGoal">添加</el-button>
        </template>
      </el-dialog>
...
const navItems = [
  ...
  { key: 'health', label: '健康数据', icon: '♥' },
  { key: 'family', label: '家庭成员', icon: '👨‍👩‍👧' },
  { key: 'checkin', label: '健康打卡', icon: '✓' },
  { key: 'favorites', label: '我的收藏', icon: '★' },
  { key: 'notifications', label: '消息通知', icon: '🔔' },
  { key: 'tools', label: '健康工具', icon: '🔧' },
...
const pageMap = {
  ...
  health: ['健康数据', '记录血压、血糖、体重等健康指标，追踪健康趋势。'],
  family: ['家庭成员', '管理家庭成员的健康档案，方便快捷地问诊。'],
  checkin: ['健康打卡', '每日健康打卡，养成良好的生活习惯。'],
  favorites: ['我的收藏', '收藏的知识、科室、报告等内容集中管理。'],
  notifications: ['消息通知', '查看系统消息、预约提醒和健康提醒。'],
  tools: ['健康工具', 'BMI计算、血压评估等健康小工具。'],
...
const healthRecordType = ref('')
const healthLoading = ref(false)
const healthDialogVisible = ref(false)
const healthForm = ref({ record_type: '', value: null, note: '', family_member_id: null })

const familyMembers = ref([])
const familyDialogVisible = ref(false)
const editingFamilyMember = ref(null)
const familyForm = ref({ name: '', gender: '', age: null, relation: '', phone: '', chronic_diseases: '', allergy_history: '' })

const checkins = ref([])
const checkinTypes = ['喝水', '运动', '睡眠', '饮食', '服药', '血压', '血糖', '其他']
const checkinDialogVisible = ref(false)
const checkinForm = ref({ checkin_type: '', note: '', mood: '', family_member_id: null })
const checkinLoading = ref(false)
const checkinStats = ref(0)

const favorites = ref([])
const favoriteType = ref('')
const favoriteDialogVisible = ref(false)
const favoriteForm = ref({ fav_type: '', title: '', content: '' })
...
const toolForm = ref({ height: 170, weight: 65, systolic: 120, diastolic: 80, bloodGlucose: 5.0, glucoseTime: '空腹' })

const healthGoals = ref([
  { id: 1, name: '每日步数', target: 10000, current: 6500, unit: '步', progress: 65 },
  { id: 2, name: '每日饮水量', target: 2000, current: 1500, unit: 'ml', progress: 75 },
  { id: 3, name: '每周运动次数', target: 5, current: 3, unit: '次', progress: 60 },
])
const goalDialogVisible = ref(false)
const goalForm = ref({ name: '', target: null, unit: '' })
...
const ChatScene = defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    const allowAttachments = props.scene !== 'triage'
    ...
    async function onPickFiles(evt) {
      if (!allowAttachments) {
        ElMessage.info('智能分诊不使用附件报告，请直接描述症状')
        return
      }
        ...
        h('div', { class: ['bubble-row', m.role], key: i }, [
          ...
          h('div', { class: 'bubble-col' }, [
            ...
            (m.streaming && m.thinking_status && m.content)
              ...
              : null,
            (m.role === 'assistant' && !m.streaming && (m.risk_level || m.recommended_department))
...
function sceneTitle(scene) {
  return { triage: '智能分诊', consultation: '线上问诊', medication: '用药咨询' }[scene] || '对话'
}
...
function openAddHealthRecord() {
  healthForm.value = { record_type: '', value: null, note: '', family_member_id: null }
  healthDialogVisible.value = true
}
async function loadHealthRecords() {
  try {
    healthRecords.value = await getHealthRecords({ record_type: healthRecordType.value || null })
  } catch { healthRecords.value = [] }
}
async function submitHealthRecord() {
  if (!healthForm.value.record_type || healthForm.value.value == null) {
    ElMessage.warning('请选择类型并输入数值')
    return
  }
  healthLoading.value = true
  try {
    const unit = healthForm.value.record_type === '血压' ? 'mmHg' : healthForm.value.record_type === '血糖' ? 'mmol/L' : 'kg'
    await createHealthRecord({
      record_type: healthForm.value.record_type,
      value: healthForm.value.value,
      unit: unit,
      note: healthForm.value.note,
      family_member_id: healthForm.value.family_member_id
    })
...
function openAddFamilyMember() {
  ...
  familyForm.value = { name: '', gender: '', age: null, relation: '', phone: '', chronic_diseases: '', allergy_history: '' }
  familyDialogVisible.value = true
}
function editFamilyMember(member) {
  editingFamilyMember.value = member
  familyForm.value = { name: member.name, gender: member.gender || '', age: member.age, relation: member.relation || '', phone: member.phone || '', chronic_diseases: member.chronic_diseases || '', allergy_history: member.allergy_history || '' }
  familyDialogVisible.value = true
}
async function loadFamilyMembers() {
  try { familyMembers.value = await getFamilyMembers() } catch { familyMembers.value = [] }
}
async function submitFamilyMember() {
  if (!familyForm.value.name) { ElMessage.warning('请输入姓名'); return }
  healthLoading.value = true
  try {
    if (editingFamilyMember.value) {
      await updateFamilyMember(editingFamilyMember.value.id, familyForm.value)
      ElMessage.success('已更新')
    } else {
      await createFamilyMember(familyForm.value)
      ElMessage.success('已添加')
    }
    familyDialogVisible.value = false
    await loadFamilyMembers()
  } catch { ElMessage.error('操作失败') } finally { healthLoading.value = false }
}
...
function openCheckinDialog() {
  checkinForm.value = { checkin_type: '', note: '', mood: '', family_member_id: null }
  checkinDialogVisible.value = true
}
function quickCheckin(type) {
  checkinLoading.value = true
  createCheckin({ checkin_type: type, note: '', mood: '', family_member_id: null }).then(() => {
...
function getCheckinTagType(type) {
  return { '喝水': 'primary', '运动': 'success', '睡眠': 'info', '饮食': 'warning', '服药': 'danger', '血压': 'warning', '血糖': 'warning' }[type] || ''
}
async function loadCheckins() {
  try { 
    ...
    if (checkins.value.length) {
      ...
      const sorted = [...checkins.value].sort((a, b) => new Date(b.checked_at || b.created_at) - new Date(a.checked_at || a.created_at))
      const dateSet = new Set(sorted.map(c => (c.checked_at || c.created_at).slice(0, 10)))
      for (let i = 0; i < 30; i++) {
    ...
    checkinStats.value = streak
  } catch { checkins.value = [] }
}
async function submitCheckin() {
  if (!checkinForm.value.checkin_type) { ElMessage.warning('请选择打卡类型'); return }
  checkinLoading.value = true
  try {
    await createCheckin({
      checkin_type: checkinForm.value.checkin_type,
      note: checkinForm.value.note,
      mood: checkinForm.value.mood,
      family_member_id: checkinForm.value.family_member_id
    })
    ElMessage.success('打卡成功！')
    checkinDialogVisible.value = false
    await loadCheckins()
    await loadFamilyMembers()
  } catch { ElMessage.error('打卡失败') } finally { checkinLoading.value = false }
}
...
async function loadFavorites() {
  try { favorites.value = await getFavorites({ fav_type: favoriteType.value || null }) } catch { favorites.value = [] }
}
async function submitFavorite() {
  if (!favoriteForm.value.fav_type || !favoriteForm.value.title) { ElMessage.warning('请选择类型并输入标题'); return }
  try {
    await createFavorite({
      fav_type: favoriteForm.value.fav_type,
      title: favoriteForm.value.title,
      content: favoriteForm.value.content
    })
    ElMessage.success('已收藏')
    favoriteDialogVisible.value = false
    favoriteForm.value = { fav_type: '', title: '', content: '' }
    await loadFavorites()
...
function getNotifIcon(type) {
  return { 'info': 'ℹ️', 'warning': '⚠️', 'success': '✅', 'error': '❌', 'appointment': '📅', 'health': '💊' }[type] || '📢'
}
async function loadNotifications() {
  try {
    const data = await getNotifications({ notif_type: notificationType.value || null })
    notifications.value = data.notifications || []
    unreadCount.value = data.unread_count || 0
  } catch { notifications.value = [] }
}
function getNotifTypeName(type) {
  const types = { 'health': '健康警告', 'appointment': '预约提醒', 'system': '系统消息' }
...
async function calcBMI() {
  try {
    bmiResult.value = await calculateBMI(toolForm.value.height, toolForm.value.weight)
  } catch { ElMessage.error('计算失败') }
}
async function saveBMIResult() {
  try {
    await createHealthRecord({
      record_type: '体重',
      value: toolForm.value.weight,
      unit: 'kg',
      note: `BMI: ${bmiResult.value.bmi}, ${bmiResult.value.category}`
    })
    ElMessage.success('已保存到健康记录')
    await loadHealthRecords()
  } catch { ElMessage.error('保存失败') }
}
async function calcBP() {
  try {
    bpResult.value = await evaluateBloodPressure(toolForm.value.systolic, toolForm.value.diastolic)
  } catch { ElMessage.error('评估失败') }
}
async function saveBPResult() {
  try {
    await createHealthRecord({
      record_type: '血压',
      value: toolForm.value.systolic,
      value_extra: toolForm.value.diastolic,
      unit: 'mmHg',
      note: `${bpResult.value.category}: ${bpResult.value.advice.slice(0, 30)}`
    })
    ElMessage.success('已保存到健康记录')
    await loadHealthRecords()
  } catch { ElMessage.error('保存失败') }
}
async function calcBloodGlucose() {
  try {
    const result = await evaluateBloodGlucose(toolForm.value.bloodGlucose, toolForm.value.glucoseTime)
    glucoseResult.value = result
  } catch { ElMessage.error('评估失败') }
}
async function saveGlucoseResult() {
  try {
    await createHealthRecord({
      record_type: '血糖',
      value: toolForm.value.bloodGlucose,
      unit: 'mmol/L',
      note: `${toolForm.value.glucoseTime}, ${glucoseResult.value.category}`
    })
    ElMessage.success('已保存到健康记录')
    await loadHealthRecords()
  } catch { ElMessage.error('保存失败') }
...
function openGoalDialog() {
  goalForm.value = { name: '', target: null, unit: '' }
  goalDialogVisible.value = true
}
function submitGoal() {
  if (!goalForm.value.name || !goalForm.value.target || !goalForm.value.unit) {
...
async function loadReports() {
  ...
  reports.value = docs.map(doc => {
    ...
    if (doc.parse_status === 'failed') {
      status = '解析失败'
    } else if (doc.parse_status === 'pending') {
      status = '报告已保存'
    } else if (doc.confidence) {
      status = `AI解析 · 置信度 ${doc.confidence}${doc.duration_ms ? ` · ${(doc.duration_ms / 1000).toFixed(1)}秒` : ''}`
    }
    return {
      ...doc,
      id: doc.doc_id,
...
function sceneLabel(scene) { return ({ triage: '分诊', consultation: '问诊', medication: '用药' })[scene] || scene || '记录' }
...
function logout() {
  ...
  ;['triage', 'consultation', 'medication'].forEach(scene => localStorage.removeItem(`medical_agent_session_${scene}`))
}
